...
{
	/*
	int seedToSoil[2][3] = {{50, 98, 2}, {52, 50, 48}};
	int soilToFertilizer[3][3] = { {0, 15, 37}, {37, 52, 2}, {39, 0, 15} };
	int fertilizerToWater[4][3] = { {49, 53, 8}, {0, 11, 42}, {42, 0, 7}, {57, 7, 4} };
	int waterToLight[2][3] = { {88, 18, 7}, {18, 25, 70} };
	int lightToTemperature[3][3] = { {45, 77, 23}, {81, 45, 19}, {68, 64, 13} };
	int temperatureToHumidity[2][3] = { {0, 69, 1}, {1, 0, 69} };
	int humidityToLocation[2][3] = { {60, 56, 37}, {56, 93, 4} };
	*/

	long long seedToSoil[40][3] = { {1615836342, 1401909974, 23067952},
	...
	long long soilToFertilizer[25][3] = { {4245401761, 2352458099, 28057201},
	...
	long long fertilizerToWater[42][3] = { {3217858280, 3761663130, 355893932},
	...
	long long waterToLight[47][3] = { {1071107509, 759231097, 26724064},
	...
	long long lightToTemperature[16][3] = { {2906633798, 3843376160, 451591136},
	...
	long long temperatureToHumidity[16][3] = { {3966168141, 3406025946, 214996780},
	...
	long long humidityToLocation[12][3] = { {848612454, 2250862530, 61410922},
	...
	// Seed to soil
	std::vector<Map> seedToSoilMaps;
	for (int i = 0; i < 40; i++)
	{
		Map newMap{ seedToSoil[i][0], seedToSoil[i][1], seedToSoil[i][2] };
		seedToSoilMaps.push_back(newMap);
	}

	// Soil to fertilizer
	std::vector<Map> soilToFertilizerMaps;
	for (int i = 0; i < 25; i++)
	{
		Map newMap{ soilToFertilizer[i][0], soilToFertilizer[i][1], soilToFertilizer[i][2] };
		soilToFertilizerMaps.push_back(newMap);
	}

	// Fertilizer to water
	std::vector<Map> fertilizerToWaterMaps;
	for (int i = 0; i < 42; i++)
	{
		Map newMap{ fertilizerToWater[i][0], fertilizerToWater[i][1], fertilizerToWater[i][2] };
		fertilizerToWaterMaps.push_back(newMap);
	}

	// Water to light
	std::vector<Map> waterToLightMaps;
	for (int i = 0; i < 47; i++)
	{
		Map newMap{ waterToLight[i][0], waterToLight[i][1], waterToLight[i][2] };
		waterToLightMaps.push_back(newMap);
	}

	// Light to temperature
	std::vector<Map> lightToTemperatureMaps;
	for (int i = 0; i < 16; i++)
	{
		Map newMap{ lightToTemperature[i][0], lightToTemperature[i][1], lightToTemperature[i][2] };
		lightToTemperatureMaps.push_back(newMap);
	}

	// Temperature to humidity
	std::vector<Map> temperatureToHumidityMaps;
	for (int i = 0; i < 16; i++)
	{
		Map newMap{ temperatureToHumidity[i][0], temperatureToHumidity[i][1], temperatureToHumidity[i][2] };
		temperatureToHumidityMaps.push_back(newMap);
	}

	// Temperature to humidity
	std::vector<Map> humidityToLocationMaps;
	for (int i = 0; i < 12; i++)
	{
		Map newMap{ humidityToLocation[i][0], humidityToLocation[i][1], humidityToLocation[i][2] };
		humidityToLocationMaps.push_back(newMap);
	}

	long long seeds[20] = { 3139431799, 50198205, 3647185634, 110151761, 2478641666, 139825503, 498892555, 8913570, 961540761, 489996751, 568452082, 100080382, 907727477, 42158689, 1617552130, 312026427, 342640189, 97088268, 2049289560, 336766062 };
	for (int seed = 0; seed < 20; seed++)
	{
		// Find seed
		long long currentSeed = seeds[seed];

		long long soilValue = currentSeed;
		for (const auto map : seedToSoilMaps)
		{
			if (currentSeed >= map.sourceStart && currentSeed <= map.sourceStart + map.rangeLength)
			{
				soilValue = (currentSeed - map.sourceStart) + map.destStart;
				break;
			}
		}

		long long fertilizerValue = soilValue;
		for (const auto map : soilToFertilizerMaps)
		{
			if (soilValue >= map.sourceStart && soilValue <= map.sourceStart + map.rangeLength)
			{
				fertilizerValue = (soilValue - map.sourceStart) + map.destStart;
				break;
			}
		}

		long long waterValue = fertilizerValue;
		for (const auto map : fertilizerToWaterMaps)
		{
			if (fertilizerValue >= map.sourceStart && fertilizerValue <= map.sourceStart + map.rangeLength)
			{
				waterValue = (fertilizerValue - map.sourceStart) + map.destStart;
				break;
			}
		}

		long long lightValue = waterValue;
		for (const auto map : waterToLightMaps)
		{
			if (waterValue >= map.sourceStart && waterValue <= map.sourceStart + map.rangeLength)
			{
				lightValue = (waterValue - map.sourceStart) + map.destStart;
				break;
			}
		}

		long long temperatureValue = lightValue;
		for (const auto map : lightToTemperatureMaps)
		{
			if (lightValue >= map.sourceStart && lightValue <= map.sourceStart + map.rangeLength)
			{
				temperatureValue = (lightValue - map.sourceStart) + map.destStart;
				break;
			}
		}

		long long humidityValue = temperatureValue;
		for (const auto map : temperatureToHumidityMaps)
		{
			if (temperatureValue >= map.sourceStart && temperatureValue <= map.sourceStart + map.rangeLength)
			{
				humidityValue = (temperatureValue - map.sourceStart) + map.destStart;
				break;
			}
		}

		long long locationValue = humidityValue;
		for (const auto map : humidityToLocationMaps)
		{
			if (humidityValue >= map.sourceStart && humidityValue <= map.sourceStart + map.rangeLength)
			{
				locationValue = (humidityValue - map.sourceStart) + map.destStart;
				break;
			}
		}

		std::cout << locationValue << std::endl;
	}
}
